...
export const registerQueue = (
  ...
  const handlerDeps = container => ({ container, options, name, parserName, processorName })
  const queueDeps = c => ({
...
export const registerQueues = (container, queues = {}, defaults = {}) => {
  ...
  container.register({
    ...
    sqsConsumerQueues: asFunction(createSqsQueues).inject(sqsConsumerQueues).scoped(),
...
const sqsQueuesStart = ({ sqsConsumerQueues }) => doSqsQueues('start', sqsConsumerQueues)
const sqsQueuesStop = ({ sqsConsumerQueues }) => doSqsQueues('stop', sqsConsumerQueues)
...
const getConsumerQueues = compose(reject(propEq('consumer', false)), getEnabledQueues)
