...
const createHandler = ({
  ...
  return process(parseBody(body, message), subcontainer)
}
...
export const registerQueue = (container, queue = {}) => {
  ...
  container.register({
    ...
    [handlerName]: asFunction(createHandler)
      .inject(handlerDeps)
      .scoped(),
    [queueName]: asFunction(createQueue)
      .inject(queueDeps)
      .singleton()
  })
}
...
export const registerQueues = (container, queues = {}, defaults = {}) => {
  ...
  container.register({
    sqsQueues: asFunction(createSqsQueues)
      .inject(sqsQueues)
      .scoped(),
